...
class Preprocess:
    ...
    def split_data(self, df, ratio=0.7, shuffle=True, seed=0):
        ...
        # self.args.train_start_row_id_by_user_id, self.args.train_user_id_row_id_list = self.save_row_ids(train)
        # self.args.valid_start_row_id_by_user_id, self.args.valid_user_id_row_id_list = self.save_row_ids(valid)
...
def get_row_ids(df, max_seq_len):
    ...
    for user_id, row_ids in question_row_ids_by_user_id.items():
        ...
        user_id_row_id_start_end.append([user_id, target_row_ids[0], target_row_ids[-1]])
...
class DKTDataset(torch.utils.data.Dataset):
    def __init__(self, data, args):
        ...
        self.user_id_row_id_list = get_row_ids(data, self.max_seq_len)
    ...
    def __getitem__(self, index):
        ...
        # target = self.target[end_row_id-1]
        mask[-seq_len:] = 1     
