...
def prepare_dataset(device, basepath, verbose=True, logger=None, ratio=0.3):
...
def train_valid_split(data, ratio):
    ...
    train_data = dict(edge=edge[:, train_idx], label=label[train_idx])
    valid_data = dict(edge=edge[:, valid_idx], label=label[valid_idx])
