...
class Ensemble:
    ...
    # Trimmed 
    # 최대, 최소 예측 확률을 제외하고 평균 앙상블을 진행합니다.
    def trimmed(self):
        return self.output_df.drop('id', axis=1).apply(lambda x: (sum(x)-max(x)-min(x))/(len(x)-2), axis=1).tolist()
    ...
    def median(self):
        return self.output_df.drop('id', axis=1).apply(lambda x: np.median(x), axis=1).tolist()
...
def main(args):
    ...
    elif args.ENSEMBLE_STRATEGY == 'TRIMMED':
        strategy_title = args.ENSEMBLE_STRATEGY.lower() #trimmed
        result = en.trimmed()
...
if __name__ == "__main__":
    ...
    arg('--ENSEMBLE_STRATEGY', type=str, default='WEIGHTED',
        choices=['WEIGHTED','TRIMMED','MEDIAN'],
        help='optional: [TRIMMED, WEIGHTED] 중 앙상블 전략을 선택해 주세요. (default="WEIGHTED")')
