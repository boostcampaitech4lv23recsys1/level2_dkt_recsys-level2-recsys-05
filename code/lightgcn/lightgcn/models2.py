...
from .model import LightGCN_context


def build(n_node, offset, weight=None, logger=None, **kwargs):
    model = LightGCN_context(n_node, offset, **kwargs)
    if weight:
...
def train(
    ...
    #     edge, label, feature = train_data["edge"], train_data["label"], train_data["feature"]
    #     label = label.to("cpu").detach().numpy()
    #     valid_data = dict(edge=edge[:, eids], label=label[eids], feature=feature[:,eids])

    logger.info(f"Training Started : n_epoch={n_epoch}")
    best_auc, best_epoch = 0, -1
    for e in range(n_epoch):
        # forward
        pred = model(train_data["edge"], train_data["cate_features"], train_data["cont_features"])
        ...
        with torch.no_grad():
            prob = model.predict_link(valid_data["edge"], valid_data["cate_features"], valid_data["cont_features"], prob=True)
...
def inference(model, data, logger=None):
    model.eval()
    with torch.no_grad():
        pred = model.predict_link(data["edge"], data["cate_features"], data["cont_features"], prob=True)
        return pred
